...
class PopupActions{
    ...
    try{
      ...
      if (stateTemp === 'NORMAL') {
        ...
        Orientation.getOrientation((err, orientation)=>{
          Debug.log2('Popup:Orientation.getOrientation',err,Debug.level.ERROR)
  ...
  setRenderContentAndShow(
    ...
    var config = Object.assign({}, RenderContent.config);
    var containerStyle = _.merge(RenderContent.containerStyle,containerStyleIn) ;
    var {
      group,
      priority,
      ...
      tapToExit,
      videoMotion
    } = _.merge( Util.formatObject(config,argFormat) , argIn);
    ...
    Debug.log2('pushPopup',RenderContent.namePopup,Debug.level.USER_TRACKER);
    self.renderStack.forEach((current,index)=>{
      Debug.log('Group '+ index + ':' + current.length );
    })
    ...
    self.renderStack[group].push({
                            info: new RenderContentInfo(
                                ...
                                },
                                {
                                  ...
                                  namePopup:RenderContent.namePopup,
    ...
    <View
      ...
      <Animatable.View
          ref={(objRef)=>{
            Debug.log('Popup: Animatable.View ref')
            if (contentObject) {
...
module.exports = {Popup,popupActions,popupConst};
